...
function onResultsHolistic(results) {
  ...
  // ISL detection integration
  // 1. Extract keypoints from results
  function extractKeypoints(results) {
    const keypoints = [];
    if (results.poseLandmarks) {
      results.poseLandmarks.forEach(lm => keypoints.push(lm.x, lm.y, lm.z || 0));
    }
    if (results.rightHandLandmarks) {
      results.rightHandLandmarks.forEach(lm => keypoints.push(lm.x, lm.y, lm.z || 0));
    }
    if (results.leftHandLandmarks) {
      results.leftHandLandmarks.forEach(lm => keypoints.push(lm.x, lm.y, lm.z || 0));
    }
    if (results.faceLandmarks) {
      results.faceLandmarks.forEach(lm => keypoints.push(lm.x, lm.y, lm.z || 0));
    }
    return keypoints;
  }

  // 2. Send keypoints to backend for prediction
  ...
  // 3. Overlay prediction on video
  (async () => {
    const keypoints = extractKeypoints(results);
    const prediction = await getISLPrediction(keypoints);
    if (prediction !== null) {
      canvasCtx4.font = '24px Arial';
      canvasCtx4.fillStyle = 'yellow';
      canvasCtx4.fillText(`ISL Prediction: ${prediction}`, 10, 40);
    }
  })();
